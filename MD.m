...
% Add global parameters for the 3rd atom type
global Mass2 Pty2in nAtoms2 T2

% Atoms position in x and y
% Vectors: x, y, Vx, Vy, Fx, Fy
% 2 atom types: local indexing (Pty0in) to select by types 
...
%% Set simulation by calling Init script
MaxX = 0;
MinX = 0;
MaxY = 0;
MinY = 0;
...
% In each of the simulation above have Add*AtomicArray.m
% The function use to add atoms to the simulation: 
% - Position
% - Random pertubation
% - Velocity
% - Temperature
% Position might not be the equilibrium position (initial disturbance)
% Velocity has 2 components: group (mass moving) and random (temperature)
...
while t < TStop
    ...
    GetForces(PhiCutoff,LJEpsilon,LJSigma); % Calculate the new forces
    ...
    % Method 2: Verlet --> More accurate way of calculating velocities
    elseif Method == 'VE'
